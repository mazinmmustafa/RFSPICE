...
class Plane():
    def __init__(self, W=0.0, H=0.0, sigma=0.0):
        self.W = W
        self.H = H
        self.sigma = sigma
        pass
    ...
    def check(self):
        try:
            ...
            for i in range(0, self.N):
                assert(self.wires[i].index==i+1)
                if self.wires[i].index==i:
                    exit(1)
                continue
            for i in range(0, self.N):
        ...
        if flag:
            self.domain*=5
            pass
        self.plane.W = self.domain*1.8
        self.plane.H = self.domain/10
        pass
...
def solveC0Matrix(config, view=True):
    ...
    file.write("\n")
    file.write("// Plane 0\n".format(eps0))
    file.write("real W={:21.14E};\n".format(config.plane.W))
    file.write("real H={:21.14E};\n".format(config.plane.H))
    file.write("\n")
    ...
    file.write("\n")
    file.write("// Mesh\n")
    file.write("border C01(t=-W/2.0, +W/2.0){x=t; y=-H;}\n")
    file.write("border C02(t=-H, 0){x=W/2.0; y=t;}\n")
    file.write("border C03(t=-W/2.0, +W/2.0){x=-t; y=0;}\n")
    file.write("border C04(t=0, H){x=-W/2.0; y=-t;}\n")
    for wire in config.wires:
    ...
    file.write("mesh Th=buildmesh(Cd(NsAir)+C01(-NsCond)+C02(-NsCond)+C03(-NsCond)+C04(-NsCond)\n")
    for wire in config.wires:
        file.write("\t+C{}(-NsCond)\n".format(wire.index))
        continue
    file.write("\t);\n")
    ...
    file.write("\t\t\t+on(C01, u=-0.5)\n")
    file.write("\t\t\t+on(C02, u=-0.5)\n")
    file.write("\t\t\t+on(C03, u=-0.5)\n")
    file.write("\t\t\t+on(C04, u=-0.5)")
    for i in range(0, config.N):
        file.write("\n\t\t\t+on(C{}, u=Vm({}, 0))".format(i+1, i))
        continue
    file.write(";\n")
    file.write("\t\t\tif (j==0)\n")
...
def solveRMatrix(config, view=True):
    ...
    file.write("real W={:21.14E};\n".format(config.plane.W))
    file.write("real H={:21.14E};\n".format(config.plane.H))
    file.write("real sigma0={:21.14E};\n".format(config.plane.sigma0))
    file.write("\n")
    for wire in config.wires:
        file.write("// Conductor {}\n".format(wire.index))
        file.write("real x{}={:21.14E};\n".format(wire.index, wire.x))
    ...
    for i in range(0, config.N):
        ...
        continue
    file.write(";\n")
    file.write("\t\t\tI0=int1d(Th, C01)(sqrt(abs(dx(u)*dx(u)+dy(u)*dy(u))))+\n")
    file.write("\t\t\t   int1d(Th, C02)(sqrt(abs(dx(u)*dx(u)+dy(u)*dy(u))))+\n")
    file.write("\t\t\t   int1d(Th, C03)(sqrt(abs(dx(u)*dx(u)+dy(u)*dy(u))))+\n")
    file.write("\t\t\t   int1d(Th, C04)(sqrt(abs(dx(u)*dx(u)+dy(u)*dy(u))));\n")
    file.write("\t\t\tif (j==0)\n")
    file.write("\t\t\t\t{I=Rs0*(int1d(Th, C01)(abs(dx(u)*dx(u)+dy(u)*dy(u)))+\n")
    file.write("\t\t\t\t        int1d(Th, C02)(abs(dx(u)*dx(u)+dy(u)*dy(u)))+\n")
    file.write("\t\t\t\t        int1d(Th, C03)(abs(dx(u)*dx(u)+dy(u)*dy(u)))+\n")
    file.write("\t\t\t\t        int1d(Th, C04)(abs(dx(u)*dx(u)+dy(u)*dy(u))))+\n")
