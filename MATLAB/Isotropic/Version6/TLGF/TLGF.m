function[V_e,I_e,V_h,I_h]=TLGF(kp,s,z,z_,Sheet)
j           =   sqrt(-1);
%% Find Layers
m           =   SelectLayer(z_);
n           =   SelectLayer(z);
...
if n==m
    if z>=z_
        V_e         =   Vep.*(exp(-j*kzn*(z-z_))+Gamma_e_U_.*exp(j*kzn*(z-z_)));
        I_e         =   Vep.*(exp(-j*kzn*(z-z_))-Gamma_e_U_.*exp(j*kzn*(z-z_)))./Zen;
        V_h         =   Vhp.*(exp(-j*kzn*(z-z_))+Gamma_h_U_.*exp(j*kzn*(z-z_)));
        I_h         =   Vhp.*(exp(-j*kzn*(z-z_))-Gamma_h_U_.*exp(j*kzn*(z-z_)))./Zhn;
    end
    if z<z_
        V_e         =   Ven.*(exp(j*kzn*(z-z_))+Gamma_e_D_.*exp(-j*kzn*(z-z_)));
        I_e         =   -Ven.*(exp(j*kzn*(z-z_))-Gamma_e_D_.*exp(-j*kzn*(z-z_)))./Zen;
        V_h         =   Vhn.*(exp(j*kzn*(z-z_))+Gamma_h_D_.*exp(-j*kzn*(z-z_)));
        I_h         =   -Vhn.*(exp(j*kzn*(z-z_))-Gamma_h_D_.*exp(-j*kzn*(z-z_)))./Zhn;
    end
end
%% Case n<m
if n<m
